...
export default function Slider(props: Props) {
  ...
  return (
    <div className='relative' >
      <div className={`fixed z-50 right-0 ${props.headerCollapsed ? 'top-20' : 'top-36'} transition-all duration-700 ${props.isMobile ? 'bg-bb-teal' : 'slider-gradient'} ${dimensions}`} >
        {props.links.map((link) => {
          if (isLinkObject(link)) {
            return (
              <Link
                className={`block w-full p-8 text-xl hover:text-bb-tan transition-all duration-500 ${props.isMobile ? 'text-center' : 'text-right'} ${props.showSlider ? 'opacity-100' : 'opacity-0'}`}
                href={link.href}
                key={link.href}
